...
def main() -> int:
    ...
    contract = build_pick_place_contract(task_id=f"phase3-safe-{uuid4().hex[:8]}")
    result = TaskExecutor(
        robot=robot, shield=SafetyShield(), repository=repository
    ).submit_contract(contract.model_dump(mode="json"))
    events = [e.event_type for e in repository.list_audit_events(contract.task_id)]
    payload = {
        ...
        "has_stop_events": "STOP_REQUESTED" in events or "STOP_CONFIRMED" in events,
    }
