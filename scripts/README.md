...
| 脚本 | 用途 | CI 安全 | 硬件 |
| --- | --- | --- | --- |
| `verify_phase11_2_model_control.py --ci` | 模型 profile、secret、endpoint、fake OpenAI-compatible、fake Ollama、前端和启动 smoke | 是 | 否 |
| `verify_phase11_2_model_control.py --ollama` | 真实本地 Ollama 和已安装模型测试 | 环境相关 | 否 |
| `check_model_runtime.py` | 读取模型运行时状态 | 是 | 否 |

`--ci` 不访问收费云 API，不下载大型模型，不连接硬件。

## Phase 12 Final Evaluation

| 脚本 | 用途 | CI 安全 | 硬件 |
| --- | --- | --- | --- |
| `run_phase12_experiments.py --profile smoke` | 生成 Phase 12 smoke 原始运行 | 是 | 否 |
| `analyze_phase12_results.py --profile smoke` | 生成 aggregate、paired 和 statistics | 是 | 否 |
| `export_phase12_thesis_assets.py --profile smoke` | 导出图表、表格、论文素材和 demo bundle | 是 | 否 |
| `verify_phase12.py --smoke` | 验证 Phase 12 smoke artifact 和安全边界 | 是 | 否 |
| `verify_phase12.py --full` | 验证 full profile 是否具备最终封板条件 | 资源相关 | 否 |

Smoke 只声明 `PHASE12_EXPERIMENT_SUITE_READY`，不能替代 full final accepted。
