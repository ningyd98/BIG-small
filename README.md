...
BIG-small 是一个面向边缘智能场景的小型机械臂云边协同控制系统，采用”云端智能规划、边缘安全执行”架构。当前版本完成 Phase 0–6.1：仓库初始化、配置、数据契约、结构化错误/日志、MockRobotAdapter、技能注册表、边缘执行运行时、可追溯状态机、边缘安全盾（21 条规则）、安全意图解析、运行时安全数据接入、云端初始规划服务、FastAPI API、EdgeGateway、Prompt 版本追踪、周期云端监督模式（PCSC），以及事件触发边缘自治闭环的真实重试、持久化、重规划和完成判定收口。Phase 5 回顾性审计见 `docs/reviews/phase5_retrospective_review.md`，Phase 6.1 收口见 `docs/phase6_1_closure_report.md`。
...
- Phase 6.1 事件触发边缘自治：`EventAutonomyRepository` 提供 InMemory/SQLite 实现，持久化事件、恢复预算、状态机、FailureSummary、CompletionSummary、LocalReplanningRequest/Result、Outbox、审计和计划版本；TaskExecutor 的 `RETRY_STEP` 真实重试同一步骤；CompletionEvaluator 阻止仅凭步骤耗尽成功；API 连接真实仓储和服务。
...
- Phase 7+：尚未实现；不得将技能缓存、AUTO 模式选择、双模式自动切换或风险调度视为当前能力。
