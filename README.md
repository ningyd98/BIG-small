...
BIG-small 是一个面向边缘智能场景的小型机械臂云边协同控制系统，采用”云端智能规划、边缘安全执行”架构。当前版本完成 Phase 0–8.1：仓库初始化、配置、数据契约、结构化错误/日志、MockRobotAdapter、技能注册表、边缘执行运行时、可追溯状态机、边缘安全盾（21 条规则）、安全意图解析、运行时安全数据接入、云端初始规划服务、FastAPI API、EdgeGateway、Prompt 版本追踪、周期云端监督模式（PCSC），事件触发边缘自治闭环的真实重试、SQLite 持久化、局部重规划 CAS apply、checkpoint 恢复和完成证据验收，Skill Cache、RiskEvaluator、AUTO 双模式选择、ModeTransition 持久化，以及可复现的仿真实验框架和 Phase 8.1 真实运行时证据闭环。Phase 8 见 `docs/phase8_experiment_design.md`、`docs/phase8_metrics.md`、`docs/phase8_reproducibility.md`、`docs/phase8_acceptance.md` 和 `docs/phase8_report.md`。Phase 8.1 见 `docs/phase8_1_design.md`、`docs/phase8_1_acceptance.md`、`docs/phase8_1_report.md` 和 `docs/phase8_1_experiment_validity.md`。
...
│   └── simulation/           # MockRobotAdapter、虚拟时钟、网络和故障注入
...
- Phase 8.1：已完成实验真实性和生产执行链路接入修复，Phase 8 实验现在通过真实 runtime chain 产出 evidence；当前仍是 mock/sim 证据，不是 Phase 9 硬件验证。
