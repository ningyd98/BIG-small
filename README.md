...
BIG-small 是一个面向边缘智能场景的小型机械臂云边协同控制系统，采用”云端智能规划、边缘安全执行”架构。当前版本完成 Phase 0–6.2：仓库初始化、配置、数据契约、结构化错误/日志、MockRobotAdapter、技能注册表、边缘执行运行时、可追溯状态机、边缘安全盾（21 条规则）、安全意图解析、运行时安全数据接入、云端初始规划服务、FastAPI API、EdgeGateway、Prompt 版本追踪、周期云端监督模式（PCSC），以及事件触发边缘自治闭环的真实重试、SQLite 持久化、局部重规划 CAS apply、checkpoint 恢复和完成证据验收。Phase 5 回顾性审计见 `docs/reviews/phase5_retrospective_review.md`，Phase 6.2 收口见 `docs/phase6_2_design.md`、`docs/phase6_2_acceptance.md` 和 `docs/phase6_2_report.md`。
