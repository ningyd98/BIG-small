...
BIG-small 是一个面向边缘智能场景的小型机械臂云边协同控制系统，采用”云端智能规划、边缘安全执行”架构。当前版本完成 Phase 0–7：仓库初始化、配置、数据契约、结构化错误/日志、MockRobotAdapter、技能注册表、边缘执行运行时、可追溯状态机、边缘安全盾（21 条规则）、安全意图解析、运行时安全数据接入、云端初始规划服务、FastAPI API、EdgeGateway、Prompt 版本追踪、周期云端监督模式（PCSC），事件触发边缘自治闭环的真实重试、SQLite 持久化、局部重规划 CAS apply、checkpoint 恢复和完成证据验收，以及 Skill Cache、RiskEvaluator、AUTO 双模式选择和 ModeTransition 持久化。Phase 7 见 `docs/skill_cache.md`、`docs/risk_policy.md`、`docs/auto_mode_selection.md`、`docs/mode_transition.md`、`docs/phase7_acceptance.md` 和 `docs/phase7_report.md`。
...
- Phase 8：尚未开始；当前没有真实硬件、真实相机模型、生产 LLM CI 或批量对比实验。
