...
- **最终评估层**：`final_evaluation` 提供 Phase 12 RQ、实验注册表、统计、图表、表格、论文素材和验收。
...
Phase 11.2 的模型控制中心只管理 Planner profile、secret 引用、endpoint 安全、Ollama 元数据和 planner dry-run。浏览器不能直接调用云模型 secret，也不能把 planner 输出直接发送硬件。

## 11. Phase 12 Final Evaluation

```mermaid
flowchart LR
  Registry[F01-F20 Registry] --> Runner[Phase12 Runner]
  Runner --> Raw[Raw Runs]
  Raw --> Aggregate[Aggregate]
  Aggregate --> Statistics[Statistics]
  Statistics --> Plots[Plots]
  Statistics --> Tables[Tables]
  Plots --> Thesis[Thesis Assets]
  Tables --> Thesis
  Thesis --> Verify[verify_phase12]
```

Phase 12 只整合软件、仿真、dry-run 和 planner dry-run 证据。`--smoke` 只验证管线，不能声明 full final accepted。
