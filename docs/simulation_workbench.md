...
Phase 11.1 将 run 和 batch 执行改为异步持久化：

- `POST /runs` 立即返回 `QUEUED`。
- SQLite 保存 job、batch、event、metric、attempt、lease 和 artifact path。
- Dashboard Live Run 展示 queue、workers、attempt、lease、cancel、timeout 和 recovery。
- WebSocket replay 使用持久 stream sequence。
- MuJoCo runtime accepted 需要 `scripts/verify_phase11_1_simulation_runtime.py --mujoco`，不能用 readiness 或 Mock fallback 代替。
