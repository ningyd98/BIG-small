...
当前权威状态：Phase 11 仿真工作台已实现，目标验收状态为 `PHASE11_SIMULATION_WORKBENCH_ACCEPTED`。Phase 11.1 追加异步编排、SQLite 持久化恢复和 MuJoCo runtime acceptance，目标状态为 `PHASE11_1_SIMULATION_RUNTIME_ACCEPTED`。Phase 10 仍保持 `PHASE10_MOVEIT_DRY_RUN_ACCEPTED`、`PHASE10_2B_CONSOLE_ACCEPTED` 和 `PHASE10_LEVEL0_FRAMEWORK_ACCEPTED`；这些状态都没有发送真实硬件执行命令。
...
Phase 11/11.1 是仿真工作台和仿真运行时，不是浏览器遥控器。它只能通过后端 API 展示 capability、场景、队列、worker、运行状态、指标、对比和 artifact，不能绕过后端直接控制硬件。真机相关开发保持冻结。
